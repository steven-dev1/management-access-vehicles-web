    ...
    <div className="space-y-6">
      ...
      <Card className="bg-slate-900/60 border-blue-800/30">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <button onClick={() => router.push('/licenses')} className="text-blue-400 text-sm hover:text-blue-300 cursor-pointer flex items-center gap-1">
          ...
          {licenses.length === 0 ? (
            ...
            <div className="space-y-2">
              {licenses.slice(0, 5).map(lic => {
                ...
                return (
                  <div key={lic.id} className="flex items-center justify-between p-3 bg-blue-950/50 rounded-lg hover:bg-blue-900/40 transition-colors cursor-pointer" onClick={() => router.push('/licenses')}>
      ...
      <Card className="bg-slate-900/60 border-blue-800/30">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <button onClick={() => router.push('/devices')} className="text-blue-400 text-sm hover:text-blue-300 cursor-pointer flex items-center gap-1">
          ...
          {devices.length === 0 ? (
            ...
            <div className="space-y-2">
              {devices.slice(0, 5).map(dev => (
                <div key={dev.id} className="flex items-center justify-between p-3 bg-blue-950/50 rounded-lg hover:bg-blue-900/40 transition-colors cursor-pointer" onClick={() => router.push('/devices')}>
                  <div className="flex items-center gap-3">
...
function UserPanel() {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Vehículos" value={stats?.total_vehicles || 0} icon={Car} color="text-blue-400" bgColor="bg-blue-500/15 border-blue-500/30" />
          ...
          <Card className="bg-slate-900/60 border-blue-800/30">
            ...
            <CardContent>
              {violations.length === 0 ? (
                ...
                <div className="space-y-2">
                  {violations.map((v) => (
                    <div key={v.apartment_code} className="flex items-center justify-between p-3 bg-blue-950/50 rounded-lg">
                      <div>
          ...
          <Card className="bg-slate-900/60 border-blue-800/30">
            ...
            <CardContent>
              {parkingAlerts.length === 0 ? (
                ...
                <div className="space-y-2">
                  {parkingAlerts.map((a) => (
                    <div key={a.vehicle_id} className="flex items-center justify-between p-3 bg-blue-950/50 rounded-lg">
                      <div>
...
function StatCard({ title, value, icon: Icon, color, bgColor, onClick }: {
  ...
  return (
    <Card
      className={`${bgColor || 'bg-blue-500/15 border-blue-500/30'} ${onClick ? 'cursor-pointer hover:scale-[1.02] transition-transform' : ''}`}
      onClick={onClick}
    >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-24 bg-slate-800/60 rounded-lg animate-pulse" />
        ))}
      </div>
      <div className="h-96 bg-slate-800/60 rounded-lg animate-pulse" />
    </div>
  );
}
